...
export function ValidateAccountForm(account: Account): boolean {
    ...
    const validFranchises: boolean = franchises.every(x => {
        return Object.entries(x).every(([key, value]) => {
            if (key.includes('Branches') || key.includes('CreatedDate') || key.includes('FranchiseCode')) return true;
            return value !== null && value !== undefined && value !== '';
        })
    });
    ...
    const validBranches: boolean = franchises.every(x => {
        return x.Branches.every(x => {
            return Object.entries(x).every(([key, value]) => {
                if(key.includes('CreatedDate') || key.includes('BranchCode')) return true;
                return value !== null && value !== undefined && value !== '';
            })
        })
    });
